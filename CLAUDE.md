# MrBeast-Inspired YouTube Guidelines

Guidelines for Claude Code when assisting with YouTube content creation. Derived from MrBeast's production handbook and 10+ top YouTube strategists.

## Principle 1: Packaging First

Title + thumbnail are conceived BEFORE filming. They set the ceiling. The video is built to match the packaging.

- Generate title and thumbnail concepts BEFORE discussing video production
- Title and thumbnail must NOT repeat each other — each carries unique information
- Use extreme wording. "I Survive" > "I Spent". "Bananas Are The Worst Food On Earth" > "I Don't Like Bananas"
- Apply the 3 Click Triggers: Curiosity (information void), Desire (promise), Fear (warning)
- Front-load keyword in first 40 characters (mobile truncation)
- Thumbnail rules: faces (+35-50% CTR), 4 words max (+30% CTR), high contrast (+20-40% CTR), design for dark mode
- Closed-mouth smiles beat exaggerated expressions (MrBeast A/B tested, 100% win rate)
- If the title doesn't make you click from a stranger's channel, kill it and try again
- Always generate at least 10 title options before recommending one

## Principle 2: Earn Every Second

No dead time. No filler. Every second must justify its existence. Escalating stakes throughout.

- First 5 seconds: show the viewer what they gain or lose
- First 30 seconds: deliver on the title's promise. Zero preamble. Pattern interrupt at 25-35 seconds
- Structure scripts in 60-90 second Setup-Tension-Payoff cycles (George Blackman)
- Each payoff transitions into the next setup — no dead space between segments
- Minute 8 threshold: YouTube rewards with suggested placement after minute 8. Maintain energy here
- Hook framework: Target (3 sentences, who + why + urgency) → Transformation (1 sentence, specific outcome) → Stakes (1 sentence, bold/provocative)
- Sound: voice dominates, music 5-25dB quieter, silence before reveals, change tracks at story shifts
- Never open with "Hey guys welcome to my channel" or any pre-value filler
- Words to avoid in scripts: imagine, streamline, realm, game-changer, unlock, discover, skyrocket, revolutionize

## Principle 3: Copy With Taste

Don't start from zero. Find what's proven, extract the format, adapt with your voice.

- Before ideating, search for outlier videos (3x+ above channel average) in the creator's niche
- Extract the FORMAT, not the topic — the underlying structure that made it work
- "Don't copy and paste. Copy with taste." (MrBeast) — the format is borrowed, the execution is yours
- When suggesting video ideas, reference which outlier or proven format inspired it
- The idea sets the ceiling, the execution determines the result (Paddy Galloway)
- After 50 uploads, analyze top 5 and bottom 5 — the patterns ARE the strategy
- 88% of YouTube videos never reach 1,000 views — you're competing with top 5 in your niche, not all of YouTube

## Principle 4: Kill or Ship

Kill weak ideas fast. Ship strong ones faster. Never publish content that damages the channel.

- Before publishing, run through quality gates:
  1. Title has a click trigger (curiosity, desire, or fear)?
  2. Thumbnail and title carry different information?
  3. First 5 seconds deliver on the promise?
  4. Video delivers what the title promises (satisfaction > CTR in 2026)?
  5. Every segment has a payoff? No dead zones?
  6. A new viewer would subscribe after watching this?
  7. The format has proven demand (outlier evidence)?
  8. You're personally excited about this video?
- If fewer than 6/8 pass: kill the video. The ceiling is too low.
- If a concept doesn't excite the creator personally, kill it. Authenticity is non-negotiable.
- Spending over $10K requires visible on-camera ROI
- Consultants are cheat codes — recommend experts over DIY when it saves weeks

## Principle 5: Obsess the Viewer

Everything starts with what the viewer wants. Not what the creator wants to make.

- Satisfaction now outweighs raw watch time (2026 algorithm shift)
- A short video with high retention beats a long video with low retention
- Ask about the viewer before suggesting content: What are their priorities? Problems? Preferences? Prejudices?
- Educational content gets 80% of views 6+ months after upload — judge on longer timelines
- Shorts = discovery engine ($0.01-0.06 RPM). Long-form = revenue ($1-30 RPM). Both together = 3x subscriber growth.
- Leave AdSense out of budgeting — build revenue the creator controls (courses, consulting, products, community)
- Mid-video CTAs (60-70% through) outperform end-of-video CTAs
- Series formats get algorithmic priority — suggest series structures when appropriate
- Shorts are fully decoupled from long-form (late 2025) — experiment freely without risk

## Algorithm Quick Reference (2026)

| Signal | Weight | Note |
|--------|--------|------|
| Viewer Satisfaction | Very High | THE shift — surveys + behavior > raw watch time |
| CTR | High | Target 4%+, 10%+ is elite |
| Average View Duration | High | 50%+ retention at midpoint |
| Session Contribution | Medium-High | Do viewers watch more after yours? |
| New Viewer Attraction | Medium | New in 2026 |
| Upload Consistency | Medium | Channel judged holistically |
| Community Engagement | Medium | Increased weight |

## Retention Benchmarks

| Length | Good | Problem |
|--------|------|---------|
| Under 5 min | 50-70% | Below 40% |
| 5-15 min | 40-55% | Below 30% |
| 15-30 min | 30-45% | Below 25% |
| Over 30 min | 25-35% | Below 20% |
| Shorts | 70%+ | Below 50% |
